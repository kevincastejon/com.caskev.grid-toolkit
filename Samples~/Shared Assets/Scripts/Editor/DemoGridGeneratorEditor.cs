...
using UnityEditor;
using UnityEngine;
namespace GridToolkitWorkingProject.Demos.Shared.Editor
{
    [CustomEditor(typeof(DemoGridGenerator))]
    public class DemoGridGeneratorEditor : UnityEditor.Editor
    {
                ...
                for (int y = 0; y < _height.intValue; y++)
                {
                    ...
                    for (int x = 0; x < _width.intValue; x++)
                    {
                        if (!_3D.boolValue)
                        {
                            ...
                            if (texture != null)
                            {
                                tile.GetComponent<ATile>().IsWalkable = !Mathf.Approximately(texture.GetPixel(x, y).r, 0f);
                            }
