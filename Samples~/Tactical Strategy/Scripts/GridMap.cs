...
namespace GridToolkitWorkingProject.Samples.TacticalStrategy
{
    public class GridMap : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            _green.Init(_map);
            _red.Init(_map);
            _green.DijkstraField = Pathfinding.GenerateDijkstraField(_map, _map[Mathf.RoundToInt(_green.transform.position.z), Mathf.RoundToInt(_green.transform.position.x)], _maxDistanceMove);
            _red.DijkstraField = Pathfinding.GenerateDijkstraField(_map, _map[Mathf.RoundToInt(_red.transform.position.z), Mathf.RoundToInt(_red.transform.position.x)], _maxDistanceMove);
        ...
        private void OnMoveMode()
        {
            ...
            Tile[] accessibleTiles = _currentCharacter.DijkstraField.GetAccessibleTiles(_map);
        ...
        private void DoMoveSelect()
        {
            ...
            if (_hoveredTile)
            {
                if (Input.GetMouseButtonDown(0) && _currentCharacter.DijkstraField.IsTileAccessible(_map, _hoveredTile) && _hoveredTile != CurrentOpponentTile)
                {
                    _lineRenderer.positionCount = 0;
                    _currentCharacter.Move(_currentCharacter.DijkstraField.GetPathFromTarget(_map, _hoveredTile));
        ...
        private void DoMove()
        {
            if (!_currentCharacter.IsMoving)
            {
                ...
                _isAttackMode = true;
                _currentCharacter.DijkstraField = Pathfinding.GenerateDijkstraField(_map, _moveDestinationTile, _maxDistanceMove);
        ...
        private void OnTileEntered()
        {
            if (!_isAttackMode)
            {
                if (_currentCharacter.DijkstraField.IsTileAccessible(_map, _hoveredTile))
                {
                    Tile[] path = _currentCharacter.DijkstraField.GetPathToTarget(_map, _hoveredTile);
